...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex justify-between items-center py-8">
        <div class="flex items-start gap-4">
            <!-- Logo -->
            <img src="{{ $post->logo ?? 'https://via.placeholder.com/80' }}" alt="{{ $post->company_name }}" class="w-20 h-20 rounded-full bg-gray-700">
            ...
            <div class="flex flex-col">
                <h2 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                    ...
                    @endif
                    <span class="text-xs font-medium text-gray-800 bg-gray-400 py-1 px-3 rounded-full">{{ $post->job_type }}</span>
                </h2>
 
                <!-- Links Row -->
                <div class="flex flex-wrap items-center gap-4 mt-2">
                    @if($post->website)
                    <div class="flex items-center gap-1 mt-1"> <!-- move to next line -->
                        <x-phosphor.icons::regular.link class="w-4 h-4 text-black flex-shrink-0" />
                        <p class="text-sm text-gray-500 break-all">
                            <a href="{{ $post->website }}" target="_blank" class="hover:underline">
                                {{ $post->website }}
                            </a>
                        </p>
                    </div>
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <p class="text-sm text-gray-500">{{ $post->phone }}</p>
                    </div>
                    @endif
                    @if($post->email)
                    <div class="flex items-center gap-1">
                        <x-phosphor.icons::regular.envelope class="w-4 h-4 text-black" />
                        <p class="text-sm text-gray-500">{{ $post->email }}</p>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="flex justify-end gap-2">
            ...
            <a href="{{ route('chat-with-company') }}" class="px-4 py-2 bg-gray-900 text-white rounded-md flex justify-start gap-2 w-50 h-10">
                Chat With Company
    ...
    <div class="mt-6 grid grid-cols-2 gap-4">
        <div class="border border-gray-100 p-4 rounded-md shadow-sm gap-2">
            ...
            <div class="py-2">
                ...
                <p class="text-sm font-medium text-gray-800">Needs:</p>
                <p class="text-sm text-gray-600">{{ $post->needs }}</p>
            </div>
        </div>
        ...
        <div class="border border-gray-100 p-4 rounded-md shadow-sm gap-2">
            ...
            <div class="py-2 flex justify-between">
                ...
                <p class="text-sm text-gray-600">{{ $post->founded_in }}</p>
            </div>
            <div class="py-2 flex justify-between">
                <p class="text-sm font-medium text-gray-800">Company Size:</p>
                <p class="text-sm text-gray-600">{{ $post->company_size }}</p>
            </div>
            <div class="py-2 flex justify-between">
                <p class="text-sm font-medium text-gray-800">Phone:</p>
                <p class="text-sm text-gray-600">{{ $post->phone }}</p>
            </div>
            <div class="py-2 flex justify-between">
                <p class="text-sm font-medium text-gray-800">Email:</p>
                <p class="text-sm text-gray-600">{{ $post->email }}</p>
            </div>
            <div class="py-2 flex justify-between">
                <p class="text-sm font-medium text-gray-800">Website:</p>
                <a href="{{ $post->website }}" class="text-sm text-blue-600 break-all">{{ $post->website }}</a>
            </div>
        </div>
    </div>
 </div>
